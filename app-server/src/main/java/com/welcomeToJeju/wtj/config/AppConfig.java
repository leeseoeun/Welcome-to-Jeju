...
// 3) JDBC 정보를 담고 있는 프로퍼티 파일을 로딩 한다.
...
public class AppConfig {
  ...
  public SqlSessionFactory sqlSessionFactory(
      DataSource dataSource,    // DB 커넥션 풀
  ...
  public MultipartResolver multipartResolver() {
    // Spring WebMVC에서 multipart/form-data 콘텐트를 리퀘스트 핸들러의 파라미터로 받고 싶다면
    // MultipartResolver 구현체를 스프링 빈 컨테이너에 등록해야 한다.
    // 그래야 요청 핸들러는 프론트 컨트롤러로부터 MultipartFile/Part 객체를 받을 수 있다.
    // MultipartResolver 객체를 등록하지 않는다면
    // HttpServletRequest를 이용하여 직접 업로드 파일의 콘텐트를 꺼내야 한다.
    // 즉, 편리하게 파라미터로 받을 수 없다.

    return new StandardServletMultipartResolver();
    // 서블릿 3.0 API의 멀티파트 처리 기능을 사용할 경우
  }

  @Bean
  public ViewResolver viewResolver() {
    // 기존의 기본 ViewResolver를 이 메서드가 리턴 하는 객체로 대체한다.
    // - 요청 핸들러가 리턴 한 jsp 이름을 가지고 앞뒤로 경로를 붙여서 찾는다.
